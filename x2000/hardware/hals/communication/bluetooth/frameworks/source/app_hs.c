...
//#define PCM_DEBUG "PCM"
struct bt_context {
	pthread_t read_bt_thread;
	pthread_t read_dmic_thread;
} bt_dev;

extern BOOLEAN avk_is_started();
...
int bt_r_fd = -1;
int bt_w_fd = -1;
BOOLEAN read_bt_call = FALSE;
BOOLEAN read_dmic_call = FALSE;

int bt_start_call(void);
int bt_end_call(void);

/*current Bluetooth Codec Select
* */
static int current_hs_bcs = BSA_SCO_CODEC_CVSD;
...
//#include "InCall.h"
//android::InCall* incall;
#ifdef __cplusplus
extern "C" {
#endif
	int construct_incall()
	{
#if 0
		if(NULL == incall){
			incall = new android::InCall();
		}
#endif
		return 0;
	}
...
void app_hs_cback(tBSA_HS_EVT event, tBSA_HS_MSG *p_data)
{
	...
	ret = open_ingenic_audio_oss_input(sample_rate);
#endif
	fprintf(stdout,"BSA_HS_AUDIO_OPEN_EVT ret %d\n", ret);
#ifdef PCM_ALSA
...
#endif
    if(current_hs_bcs==BSA_SCO_CODEC_MSBC)
	open_ingenic_audio_oss_output("hfp-msbc", 8);
    else
	open_ingenic_audio_oss_output("hfp-cvsd", 8);
	...
	if (p_conn->connection_active)
        {
		int indicator = buf[1] - '0';
		int indicator_value = buf[3] - '0';
		if(indicator == BTA_AG_IND_CALL){
			if(indicator_value == BSA_HS_CALL_ACTIVE){
				// call active
				BSA_HS_SETSTATUS(p_conn, BSA_HS_ST_CALLACTIVE);
			}else if(indicator_value == BSA_HS_CALL_INACTIVE){
				// call inactive
				BSA_HS_RESETSTATUS(p_conn, BSA_HS_ST_CALLACTIVE);
			}
		}else if(indicator == BTA_AG_IND_CALLSETUP){
			if(indicator_value == BTA_AG_CALLSETUP_INCOMING){
				// callsetup incoming
				BSA_HS_SETSTATUS(p_conn, BSA_HS_ST_RINGACT);
			}else if(indicator_value == BSA_HS_CALLSETUP_OUTGOING || indicator_value == BSA_HS_CALLSETUP_ALERTING){
				// callsetup outgoing
				BSA_HS_SETSTATUS(p_conn, BSA_HS_ST_OUTGOINGCALL);
			}else{
				// callsetup none
				BSA_HS_RESETSTATUS(p_conn, BSA_HS_ST_OUTGOINGCALL | BSA_HS_ST_RINGACT);
			}
		}
		printf("BSA_HS_CIEV_EVT: p_conn->status %x\n", p_conn->status);
		if(BSA_HS_GETSTATUS(p_conn, BSA_HS_ST_IN_CALL))
		{
			//incall state
			construct_incall();
		}else if(BSA_HS_GETSTATUS(p_conn, BSA_HS_ST_CONNECT))
		{
			//only hs is connected state
			destruct_incall();
		}
	}
        break;
...
#endif
static int config_device(int fd, int *format, UINT32 *channel, UINT32 *speed)
{
	...
	if ((ioctl(fd, SNDCTL_DSP_CHANNELS, channel)) == -1) {
		fprintf(stderr, "Set channel failed: %s\n", strerror(errno));
		return -1;
	}
...
#ifdef PCM_DEBUG
	FILE *out_fd=NULL;
	out_fd = fopen("/data/speak_out.pcm", "wb");
	if(out_fd==NULL){
	  ALOGE("out_fd Null, EXIT errno: %d, means: %s", errno, strerror(errno));
	}
#endif
	char *read_buf = (char*)malloc(AD_NODE_BUFSIZ);
	while (read_bt_call) {
		size = read(bt_r_fd, read_buf, AD_NODE_BUFSIZ);
		if (size > 0) {
		    int ret = ingenic_audio_oss_write(read_buf,size);
		    if(ret <= 0)
			APP_ERROR1("send buffer data error: %s\n",strerror(errno));
#ifdef PCM_DEBUG
			fwrite(read_buf, AD_NODE_BUFSIZ, 1, out_fd);
#endif
		}
...
{
	char *ref_buf  = (char*)malloc(AD_NODE_BUFSIZ);
	if (!ref_buf) {
		printf("Error: malloc ref_buf failed!!\n");
		return NULL;
	}
	while (read_dmic_call) {
	    if (bt_w_fd >= 0){
#ifdef USE_INGENIC_AUDIO_OSS_IN
		int ret = ingenic_audio_oss_read(ref_buf, AD_NODE_BUFSIZ);
		if(ret >= 0){
		    write(bt_w_fd, ref_buf, ret);
		}
#endif
	    }
	}
...
void close_dsp(){
	int ret = 0;
	/* Warning: must close bt_w_fd first
	 * when pthread_join read_dmic_thread */
	read_dmic_call = FALSE;
	if (bt_w_fd >= 0) {
		if (ioctl(bt_w_fd, SNDCTL_EXT_STOP_DMA, &ret) < 0)
			printf("%s ioctl set SNDCTL_EXT_STOP_DMA failed !\n", __func__);
		close(bt_w_fd);
		bt_w_fd = -1;
	}
	if (bt_dev.read_dmic_thread != 0) {
		pthread_join(bt_dev.read_dmic_thread, NULL);
		bt_dev.read_dmic_thread = 0;
	}
	read_bt_call = FALSE;
	if (bt_r_fd >= 0) {
		if (ioctl(bt_r_fd, SNDCTL_EXT_STOP_DMA, &ret) < 0)
			printf("%s ioctl set SNDCTL_EXT_STOP_DMA failed !\n", __func__);
		close(bt_r_fd);
		bt_r_fd = -1;
	}
	if (bt_dev.read_bt_thread != 0) {
		pthread_join(bt_dev.read_bt_thread, NULL);
		bt_dev.read_bt_thread = 0;
	}
}
int bt_start_call(void)
{
	int ret = 0;
	unsigned int samplerate = (current_hs_bcs==BSA_SCO_CODEC_MSBC) ? 16000:8000;
	unsigned int channel = APP_HS_CHANNEL_NB;
	int format = AFMT_S16_LE;
	/* printf("HS ori rate: %d, channel: %d, bits: %d\n", */
	/* 		hs_sample_rate, hs_sample_channel, hs_sample_bits); */

	bt_w_fd = open("/dev/dsp1", O_WRONLY);
	if(bt_w_fd < 0){
	    printf("open bt_w_fd failed!\n");
	    return -1;
	}

	bt_r_fd = open("/dev/dsp1", O_RDONLY);
	if(bt_r_fd < 0){
	    printf("open bt_r_fd failed!\n");
	    goto err_start;
	}
	if (config_device(bt_w_fd, &format, &channel, &samplerate) < 0) {
		printf("config bt_r_fd failed!\n");
		goto err_start;
	}
	if (config_device(bt_r_fd, &format, &channel, &samplerate) < 0) {
		printf("config bt_r_fd failed!\n");
		goto err_start;
	}

	read_dmic_call = TRUE;
	ret = pthread_create(&bt_dev.read_dmic_thread, NULL, read_dmic_thread_func, NULL);
	if (ret != 0) {
		printf("create read_dmic_thread error: %s\n", strerror(ret));
		goto err_start;
	}

	read_bt_call = TRUE;
	ret = pthread_create(&bt_dev.read_bt_thread, NULL, read_bt_thread_func, NULL);
	if (ret != 0) {
		printf("create read_bt_thread error: %s\n", strerror(ret));
		goto err_start;
	}
