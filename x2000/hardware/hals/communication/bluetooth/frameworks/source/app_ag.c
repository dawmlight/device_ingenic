...
#if BLUETOOTH_AG_SUPPORT
static void ingenic_bluetooth_ag_connect();
static void ingenic_bluetooth_ag_disconnect();
#endif
...
void app_ag_cback(tBSA_AG_EVT event, tBSA_AG_MSG *p_data)
{
    ...
    case BSA_AG_CLOSE_EVT: /* Connection Closed (for info)*/
        ...
        app_ag_cb.opened = FALSE;
#if BLUETOOTH_AG_SUPPORT
		ingenic_bluetooth_ag_disconnect();
#endif
        break;
...
{
	int fd;
	fd = open("/tmp/bluetooth_ag_connect", O_WRONLY| O_CREAT);
	close(fd);
...
static void ingenic_bluetooth_ag_disconnect()
{
	 unlink("/tmp/bluetooth_ag_connect");
}
#endif
