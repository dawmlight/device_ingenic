...
#define A2DP_SOURCE_SHM_KEY "a2dp"
#define A2DP_SOURCE_SHM_SIZE (64*1024)
static int a2dp_playback_context;
...
#define APP_AV_MAX_AUDIO_BUF 1024
		...
		do
		{
			...
			case APP_AV_PLAYTYPE_MIC:
			    ...
			    if(mIngenicA2dpSocketFd < 0){
					APP_DEBUG0("socket is disconnected");
			    }else{
#if 0
					static long long st = 0LL;
					static long total = 0;
#endif
					//APP_INFO1("recv in app_av_cb.uipc_cfg.length = %d\n",app_av_cb.uipc_cfg.length);

					//nb_bytes = recv(mIngenicA2dpSocketFd, app_av_cb.audio_buf,
					//					app_av_cb.uipc_cfg.length, MSG_NOSIGNAL);
					do{
						nb_bytes = SHM_Read(a2dp_playback_context,app_av_cb.audio_buf,app_av_cb.uipc_cfg.length);
						if(nb_bytes == -1)
							break;
					}while(nb_bytes != app_av_cb.uipc_cfg.length);
#if 0
					total += nb_bytes;
					//APP_INFO1("recv in nb_bytes = %d total = %d\n",nb_bytes,total);
					if(total > 2 * 1024 * 1024){
						struct timeval tv;
						long long en,diff;
						gettimeofday(&tv,0);
						en = tv.tv_sec * (1000*1000LL) + tv.tv_usec;
						diff = en - st;
						st = en;
						APP_INFO1("recv out total = %d diff = %d\n",total,diff);
						total = 0;
					}
#endif
 ...
 *******************************************************************************/
			int app_av_test_sec_codec(BOOLEAN use_prev_samp_freq)
			{
				...
				{
					app_av_cb.test_sec_sampfreq_index = 1;
...
static int ingenic_a2dp_connect(){
	...
	a2dp_playback_context = SHM_Open(A2DP_SOURCE_SHM_KEY,A2DP_SOURCE_SHM_SIZE);
	if(a2dp_playback_context == 0){
		APP_ERROR1("BluetoothManager:shm open failed: %s\n",strerror(errno));
		return -1;
	}
	return 0;
}
...
static int ingenic_a2dp_disconnect(){
	...
	if(a2dp_playback_context){
		SHM_Close(a2dp_playback_context);
	}
	return 0;
}
