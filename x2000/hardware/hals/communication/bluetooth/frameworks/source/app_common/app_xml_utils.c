...
#define APP_XML_SI_DEVICES_FILE_PATH        "/usr/data/bsa/si_devices.xml"
...
int app_write_xml_config(const tAPP_XML_CONFIG *p_xml_config)
{
    int status;
        ...
        APP_INFO1("====%s %d=================blue MAC:->%02x-%02x-%02x-%02x-%02x-%02x-",
...
char *app_xml_read_data(unsigned int *p_datalen, const char *p_buf, unsigned int length)
{
    ...
    *p_data = 0;
    ...
    for (index = 0; index < length; index++)
    {
        ...
        if ((p_buf[index] == ':') || (p_buf[index] == '\n') ||
            (isspace((unsigned)p_buf[index]) && valid))
        {
            p_data++;
