...
int app_dm_get_local_bt_config(void)
{
    ...
    APP_INFO1("\tBLE Name:%s", bt_config.ble_name);
...
int app_dm_set_local_bt_config(BOOLEAN enable)
{
    ...
    /* Do not enable InquiryScan and PageScan for the moment (because of BdAddr change) */
    bt_config.discoverable = FALSE;
    bt_config.connectable = FALSE;
    ...
    strncpy((char *)bt_config.ble_name, (char *)xml_local_config.ble_name, sizeof(bt_config.ble_name));
    bt_config.ble_name[sizeof(bt_config.ble_name)-1] = '\0';
    ...
    /* If BT is enabled and either InquiryScan or PageScan requested */
    if ((enable) &&
        (xml_local_config.discoverable || xml_local_config.connectable))
    {
        /* Init config parameter */
        status = BSA_DmSetConfigInit(&bt_config);

        /* Set InquiryScan and PageScan configuration (from XML contents) */
        bt_config.enable = enable;
        ...
        if (status != BSA_SUCCESS)
        {
            APP_ERROR1("Unable to set BT Config (Scan) status:%d", status);
            return(-1);
        }
    }
...
int app_dm_set_ble_adv_data(tBSA_DM_BLE_ADV_CONFIG *p_data)
{
    ...
    bt_config.adv_config.is_part_service = p_data->is_part_service;
    ...
    //wayne add begin
    memcpy(&bt_config.adv_config.sol_service_128b, &p_data->sol_service_128b, sizeof(tBSA_DM_BLE_128SERVICE));
    //wayne add end
 ...
 **
 *******************************************************************************/
int app_dm_get_ble_adv_param(void)
{
    int status;
    tBSA_DM_GET_CONFIG bt_config;
    ...
    APP_INFO1("ble_adv_param adv_int_min=0x%x, adv_int_max=0x%x",(UINT16)bt_config.ble_adv_param.adv_int_min, (UINT16)bt_config.ble_adv_param.adv_int_max);
 ...
 **
 *******************************************************************************/
static BOOLEAN ble_beacon_state = 0;
BOOLEAN app_dm_get_ble_visibility(void){
    return ble_beacon_state;
}
int app_dm_ensure_ble_visibility(void){
    if(!ble_beacon_state) {
