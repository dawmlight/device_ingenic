...
#define APP_XML_CONF_BLE_NAME_KEY       "ble_name"
...
int app_xml_write_cfg(const char *p_fname, const tAPP_XML_CONFIG *p_xml_config)
{
	...
	app_xml_open_tag(fd, APP_XML_CONF_BLE_NAME_KEY, FALSE);
	dummy = write(fd, (char *) p_xml_config->ble_name, strlen((char *) p_xml_config->ble_name));
	(void)dummy;
	app_xml_close_tag(fd, APP_XML_CONF_BLE_NAME_KEY, FALSE);
		...
		const char *tagName, unsigned len)
{
	...
	else if (strncmp(tagName, APP_XML_CONF_BLE_NAME_KEY, strlen(
					APP_XML_CONF_BLE_NAME_KEY)) == 0)
	{
		app_xml_param_cb.cfg_tag = CONF_BLE_NAME_KEY;
	}
		...
		unsigned len, int more)
{
	...
	switch (app_xml_param_cb.cfg_tag)
	{
		...
		case CONF_NAME_KEY:
	        ...
	        case CONF_BLE_NAME_KEY:
			app_xml_read_string(&app_xml_param_cb.config_ptr->ble_name,
					sizeof(app_xml_param_cb.config_ptr->ble_name), data, len);
			break;
		...
		int nb_device_max, BD_ADDR bd_addr)
{
	...
	for (index = 0; index < nb_device_max; index++)
	{
		...
		{
			memset(&p_stored_device_db[index], 0, sizeof(tAPP_XML_REM_DEVICE));
			/* Let's use it */
...
/*******************************************************************************
 **
 ** Function        app_xml_del_same_services_db
 **
 ** Description     del a trusted service for a device 主要用途时删除连接过的设备，每一类设备只保留一种
 **
 ** Returns         0 if successful, error code otherwise
 **
 *******************************************************************************/
//#define USELESS_SERVICE_MASK	(BSA_SPP_SERVICE_MASK | BSA_A2DP_SERVICE_MASK | BSA_AVRCP_SERVICE_MASK | BSA_BLE_SERVICE_MASK)
int app_xml_del_same_services_db(tAPP_XML_REM_DEVICE *p_stored_device_db,
		int nb_device_max, BD_ADDR bd_addr, tBSA_SERVICE_MASK trusted_services)
{
	int index,ret=-1,ret2;

	ret2 = app_xml_add_dev_db(p_stored_device_db, nb_device_max, bd_addr);
	...
	for (index = 0; index < nb_device_max; index++)
	{
		...
		{
			APP_INFO1("del services=%x,bd_addr=%02x:%02x:%02x:%02x:%02x:%02x",trusted_services,
				p_stored_device_db[index].bd_addr[0],p_stored_device_db[index].bd_addr[1],
				p_stored_device_db[index].bd_addr[2],p_stored_device_db[index].bd_addr[3],
				p_stored_device_db[index].bd_addr[4],p_stored_device_db[index].bd_addr[5]);
			app_mgr_remove_device(p_stored_device_db[index].bd_addr);
			p_stored_device_db[index].in_use = FALSE;
			ret = 0;
		}/*else if((p_stored_device_db[index].in_use != FALSE) && 
			(bdcmp(p_stored_device_db[index].bd_addr, bd_addr) != 0) &&
			(p_stored_device_db[index].trusted_services & (~USELESS_SERVICE_MASK)) )//删除非ble，非spp，非a2dp设备
		{
			APP_INFO1("del zero services=%x,bd_addr=%02x:%02x:%02x:%02x:%02x:%02x",p_stored_device_db[index].trusted_services,
				(unsigned char)p_stored_device_db[index].bd_addr[0],(unsigned char)p_stored_device_db[index].bd_addr[1],
				(unsigned char)p_stored_device_db[index].bd_addr[2],(unsigned char)p_stored_device_db[index].bd_addr[3],
				(unsigned char)p_stored_device_db[index].bd_addr[4],(unsigned char)p_stored_device_db[index].bd_addr[5]);
			app_mgr_remove_device(p_stored_device_db[index].bd_addr);
			p_stored_device_db[index].in_use = FALSE;
			ret = 0;
		}*/
		...
		int nb_device_max, BD_ADDR bd_addr, tBSA_SERVICE_MASK trusted_services)
{
	...
	ret2 = app_xml_del_same_services_db(p_stored_device_db, nb_device_max, bd_addr, trusted_services);
	ret3 = app_xml_add_dev_db(p_stored_device_db, nb_device_max, bd_addr);
	/* First look in Database if this device already exist */
	if(trusted_services != 0)
	for (index = 0; index < nb_device_max; index++)
	{
		if ((p_stored_device_db[index].in_use != FALSE) && (bdcmp(
						p_stored_device_db[index].bd_addr, bd_addr) == 0))
		{
			APP_INFO1("cur service=%x,will save service=%x,bd_addr=%02x:%02x:%02x:%02x:%02x:%02x",p_stored_device_db[index].trusted_services,trusted_services,
			...
			if((p_stored_device_db[index].trusted_services & trusted_services) != trusted_services)
			{
		...
		int nb_device_max, BD_ADDR bd_addr, BD_NAME name)
{
	...
	ret = app_xml_add_dev_db(p_stored_device_db, nb_device_max, bd_addr);
	...
	for (index = 0; index < nb_device_max; index++)
	{
		if ((p_stored_device_db[index].in_use != FALSE) && (bdcmp(
						p_stored_device_db[index].bd_addr, bd_addr) == 0))
		{
			if(strcmp(p_stored_device_db[index].name, name) != 0)
			{
				APP_INFO1("cur name=%s,will save name=%s,bd_addr=%02x:%02x:%02x:%02x:%02x:%02x",p_stored_device_db[index].name,name,
					(unsigned char)bd_addr[0],(unsigned char)bd_addr[1],
					(unsigned char)bd_addr[2],(unsigned char)bd_addr[3],
					(unsigned char)bd_addr[4],(unsigned char)bd_addr[5]);
		...
		int nb_device_max, BD_ADDR bd_addr, DEV_CLASS class_of_device)
{
	...
	ret = app_xml_add_dev_db(p_stored_device_db, nb_device_max, bd_addr);
	if((class_of_device[0]!=0) || (class_of_device[1]!=0) || (class_of_device[2]!=0))
	/* First look in Database if this device already exist */
	for (index = 0; index < nb_device_max; index++)
	{
		if ((p_stored_device_db[index].in_use != FALSE) && (bdcmp(
						p_stored_device_db[index].bd_addr, bd_addr) == 0))
		{
			if(memcmp(p_stored_device_db[index].class_of_device, class_of_device, 3) !=0)
			{
				APP_INFO1("cur cod=%02x:%02x:%02x,will save cod=%02x:%02x:%02x,bd_addr=%02x:%02x:%02x:%02x:%02x:%02x",
					(unsigned char)p_stored_device_db[index].class_of_device[0],(unsigned char)p_stored_device_db[index].class_of_device[1],(unsigned char)p_stored_device_db[index].class_of_device[2],
					(unsigned char)class_of_device[0],(unsigned char)class_of_device[1],(unsigned char)class_of_device[2],
					(unsigned char)bd_addr[0],(unsigned char)bd_addr[1],
					(unsigned char)bd_addr[2],(unsigned char)bd_addr[3],
					(unsigned char)bd_addr[4],(unsigned char)bd_addr[5]);
		...
		int nb_device_max, BD_ADDR bd_addr, UINT8 ble_addr_type)
{
	...
	for (index = 0; index < nb_device_max; index++)
	{
		if ((p_stored_device_db[index].in_use != FALSE) && (bdcmp(
						p_stored_device_db[index].bd_addr, bd_addr) == 0))
		{
			APP_DEBUG1("Update ble_addr_type:%d\n", ble_addr_type);
		...
		int nb_device_max, BD_ADDR bd_addr, tBT_DEVICE_TYPE device_type)
{
	...
	for (index = 0; index < nb_device_max; index++)
	{
						...
						p_stored_device_db[index].bd_addr, bd_addr) == 0))
		{
			...
			//return 0;
		}else if((p_stored_device_db[index].in_use != FALSE) &&(p_stored_device_db[index].device_type == device_type)) {
            p_stored_device_db[index].in_use = FALSE;
        }
