...
/* Event associated with BTA_AV_UPDATE_SEPS_EVT */
typedef struct
{
    UINT16  status;
    BOOLEAN available;
...
/* data associated with BTA_AV_DELAY_RPT_EVT */
typedef struct
{
    tBTA_AV_CHNL    chnl;
    tBTA_AV_HNDL    hndl;
    UINT16          delay;
} tBTA_AV_DELAY_RPT;        /* BSA_SPECIFIC */
...
/* Info from peer's AVRC SDP record (included in BTA_AV_RC_OPEN_EVT) */
typedef struct
{
...
/* Data for BTA_AV_RC_CMD_TOUT_EVT */
typedef struct
{
    UINT8           rc_handle;
...
typedef union
{
    ...
    tBTA_AV_UPDATE_SEPS update_seps;
    tBTA_AV_OPEN        open;
    tBTA_AV_CLOSE       close;
    tBTA_AV_DELAY_RPT   delay;          /* BSA_SPECIFIC */
    tBTA_AV_START       start;
    ...
    tBTA_AV_RC_CMD_TOUT rc_cmd_tout;
} tBTA_AV;
...
**
*******************************************************************************/
BTA_API void BTA_AvUpdateStreamEndPoints(BOOLEAN available);
