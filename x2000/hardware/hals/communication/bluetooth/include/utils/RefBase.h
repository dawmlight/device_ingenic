 ...
 * @brief RefBase类
 * RefBase类 来源于Android
 * sp是Strong pointer ,wp是Weak pointer ， 是实现android C++ 自动内存分配和回收避免内存泄漏的机制，类似于java的垃圾回收机制。
 *
 ...
 * @endcode
 *
 * @attention 注意不要定义成sp<MyClass>* p_obj。这是初学者很容易犯的错误，这样其实相当于定义了一个指针的指针。尽管在语法上没有问题，但是最好不要这样定义。
 *
 ...
 * @endcode
 *
 * @attention 注意不要试图delete一个智能指针，即 delete p_obj。不要担心对象的销毁问题，智能指针的最大作用就是自动销毁不再使用的对象。不需要再使用一个对象后，直接将指针赋值为NULL即可：
 * @code
 * p_obj = NULL;
 * @endcode
 *
