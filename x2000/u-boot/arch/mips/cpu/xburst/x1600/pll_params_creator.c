...
struct pll_control
{
	...
	unsigned int dsmpdd:1;
	unsigned int phasepd:1;
...
/* M = PLLFD 9bits 20:28bit */
/* N = PLLRD 6bits 14-19bit */
/* OD = PLLOD 3bits  11-13bit */
/* 0 <= M <= 511  12bits feedback divider */
/* 0 <= N <= 63  6bits reference divider */
/* OD 2^(1-7)  output divider */
/* OD-->NO 2 4 8 16 32 64  output divider */
/* NF = 1+M */
/* NR = 1+N */
/* NO = NO */
/*  FREF = FIN/NR */
	...
	for(n = 1; n < 64; n++) {
		for(m = 16; m < 2048; m++) {
			for(od1 = 1; od1 <8; od1++) {
				od = od0 * od1;
				fref = fin / n;
				if((fin%n) || (fref < 1) || (fref > 800)) {
					continue;
				}
				fvco = fout * od;
				if((fref*m != fvco) || (fvco < 600) || (fvco > 2400)) {
					continue;
				}
